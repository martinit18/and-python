...
void chebyshev_clenshaw_complex(const int dim_x, const int max_order, const char * restrict boundary_condition, double complex * restrict wfc, double  complex * restrict psi, double complex * restrict psi_old, const double tunneling, const double * restrict disorder, const double * restrict tab_coef, const double g_times_delta_t, const double e0_times_delta_t)
{ 
  int i, order;
  double argument;
...
#endif
    for (i=0;i<dim_x;i++) {
      argument = e0_times_delta_t+g_times_delta_t*(creal(psi[i])*creal(psi[i])+cimag(psi[i])*cimag(psi[i]));
