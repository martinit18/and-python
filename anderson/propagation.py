...
class Temporal_Propagation:
  ...
  def compute_chebyshev_coefficients(self,accuracy,my_timing):
...
# max order must be an even number
    max_order = 2*((i+1)//2)
    if (max_order>my_timing.MAX_CHE_ORDER):
      my_timing.MAX_CHE_ORDER=max_order
#  print(max_order)
...
def chebyshev_step_clenshaw_python(wfc, H, propagation):
  dim_x = H.dim_x
  ...
  if H.interaction==0.0:
    ...
    phase=propagation.delta_t*H.medium_energy+propagation.delta_t*H.interaction*(psi[0:dim_x]**2+psi[dim_x:2*dim_x]**2)
...
def chebyshev_step_clenshaw_cffi(wfc, H, propagation):
...
def gross_pitaevskii(t, wfc, H, data_layout, rhs, my_timing):
    ...
    For data_layout == 'complex':
      ...
      wfc[1:2*dim_x:2] contains the my_timing.timing(imag part of the wavefunction.
    ...
    my_timing.GPE_TIME+=(timeit.default_timer() - start_time)
    my_timing.NUMBER_OF_OPS+=16.0*H.dim_x
    return rhs

class Measurement:
  def __init__(self, delta_t_measurement, measure_density=False, measure_density_momentum=False, measure_autocorrelation=False, measure_dispersion=False, measure_dispersion_momentum=False, measure_wavefunction_final=False,measure_extended=False):
...
#    self.tab_momentum = np.zeros(0)
  ...
  def normalize(self,n_config):
    ...
    if self.measure_dispersion:
      ...
      if self.extended:
#        print(self.tab_position[0])
#        print(self.tab_position[1])
...
def gpe_evolution(i, initial_state, H, propagation, measurement, my_timing, debug=False):
...
#  init_state_autocorr = anderson.Wavefunction(dim_x,delta_x)
  ...
  if (measurement.measure_dispersion):
...
#  print(timeit.default_timer())
  if (propagation.method=='ode'):
    ...
    solver = ode(f=lambda t,y: gross_pitaevskii(t,y,H,propagation.data_layout,rhs,my_timing)).set_integrator('dop853', atol=1e-5, rtol=1e-4)
    solver.set_solout(solout)
    solver.set_initial_value(y)
  else:
#    start_dummy_time=timeit.default_timer()
    e_min, e_max = H.energy_range()
#    my_timing.DUMMY_TIME+=(timeit.default_timer() - start_dummy_time)
...
#    print(e_min,e_max)
    ...
    accuracy = 1.e-6
    propagation.compute_chebyshev_coefficients(accuracy,my_timing)
...
#     print(i_prop,t_next)
    if (propagation.method == 'ode'):
      ...
      my_timing.ODE_TIME+=(timeit.default_timer() - start_ode_time)
    else:
      start_che_time = timeit.default_timer()
#      print(i_prop,start_che_time)
      chebyshev_step(y, H, propagation)
#      print(y[2000])
      my_timing.CHE_TIME+=(timeit.default_timer() - start_che_time)
      my_timing.NUMBER_OF_OPS+=16.0*dim_x*propagation.tab_coef.size
...
#          measurement.tab_autocorrelation[i_tab-i_tab_0] = psi.overlap(init_state_autocorr)
      ...
      my_timing.EXPECT_TIME+=(timeit.default_timer() - start_expect_time)
      i_tab+=1
